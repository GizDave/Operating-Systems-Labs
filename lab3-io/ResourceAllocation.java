...
class Safer{
	...
	protected static boolean isSafe(int[] available, int[][] claims, int[][] allocations){
		...
		//check if every task is pseudo-terminated (ie allocation for each resource type is 0)
		...
		if(canTerminate)
			return true;
		canTerminate=true;
		//for each terminate-able task (ie there are enough resources for each resource claim of the task), recursively call the method until a terminateble route is found.
...
public class ResourceAllocation{
	...
	public ResourceAllocation(String contents){
		...
		//back up data
		backup_input=new LinkedList[task_input.length];
		for(i=0; i<num_task; i++){
			backup_input[i]=new LinkedList<>();
			backup_input[i].addAll(task_input[i]);
		}
	...
	private void reset(){
		...
		for(i=0; i<num_task; i++){
			task_input[i].clear();
			task_input[i].addAll(backup_input[i]);
			print_stack[i]=new data_wrap();
		}
	}
			...
			for(i=0; i<task_input.length; i++){
				...
				switch(command){
					case initiate:  //optimistic manager ignores initial claims
									...
									break;
					case request:	//copy request into the all_requests matrix
		...
		while(num_alive>0){
			...
			if(pending_requests.size()>0){
				...
				while(it.hasNext()){
					...
					//check if there are enough resources for the request
					if(Safer.isLegal(available_resource, all_requests[i])){
						//create clones for testing
						...
						//check if granting the request would lead to a safe state. if yes, then grant and remove the task from the pending list
			...
			for(i=0; i<task_input.length; i++){
				...
				command=Act.valueOf(input[0]);
				switch(command){
					case initiate:  //check if the task's claim is within the range [0, max of said resource type in system]
									//System.out.println("	"+original+" initiate");
									request=Safer.loadRequest(input, num_type);
									...
									if(Safer.isLegal(existing_resource, all_requests[i])){
										...
										System.out.print("Warning: task "+(i+1)+"'s claim exceeds the resources present. ");
										task_input[i].clear();
										task_input[i].addFirst("abort");
										num_alive--;
					...
					case request:	//copy request into the all_requests matrix
									...
									for(j=0; j<all_claims[0].length; j++)
										if(request[j]+current_allocation[i][j]>all_claims[i][j]){
											System.out.print("Warning: task "+(i+1)+"'s request exceeds its claim. ");
											all_releases[i][0]=1;
											for(j=1; j<all_releases[i].length; j++)
												all_releases[i][j]=current_allocation[i][j-1];
											//Safer.release(available_resource, current_allocation[i], all_releases[i]);
									...
									break;
					case compute:	//compute for one cycle
									//System.out.println("	"+original+" compute");
									if((j=Integer.parseInt(input[2]))>1){
										task_input[i].addFirst(String.format("compute %s %d 0", input[1], j-1));
										//System.out.println("	task "+(i+1)+" is computing.");
									}
					...
					case release:	//copy release into the all_releases matrix
									...
									break;
					case terminate: //terminates task
									//System.out.println("	"+original+" terminate");
									task_input[i].clear();
									print_stack[i].time_taken=cycles;
									num_alive--;
									break;
			...
			//remove pending requests that were granted at the beginning of the cycle
			if(remove_these.size()>0){				
				pending_requests.removeAll(remove_these);
				remove_these.clear();
			}
