...
public class DemandPaging{
	...
	private void driver() throws IOException{
		...
		for(i=0; i<procs.length; i++){
			...
			System.out.println(String.format("Process %d has %d faults, %d evictions, and %s average_residency.", i+1, procs[i].page_fault, procs[i].eviction, (procs[i].eviction>0)?Double.toString(procs[i].average_residency/procs[i].eviction):"undefined"));
		}
	...
	public static void main(String[] args){
		...
		if(args.length==1){
			...
			if(args[0].equals("lru"))
				collection=new int[]{3};//{1,2,3,4,7,9,11,12,14};
			else if(args[0].equals("random"))
				collection=new int[]{5};//{5,10,15};
			else if(args[0].equals("lifo"))
				collection=new int[]{6};//{6,8,13,16};
